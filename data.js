...
},
{
  ...
  ans: "检索侧缓解：提升检索质量（混合检索+Rerank+相似度阈值）、Query 改写、多路召回（向量+关键词+知识图谱）、元数据过滤缩小范围。生成侧缓解：用 Prompt 强约束（“仅基于以下检索内容回答，无法回答时说不知道”）、要求引用标注来源段落、相似度低于阈值时拒绝回答、用 LLM-as-Judge 或规则做后置校验、降低 temperature。核心认知是：幻觉根因要么是“检索到了但模型没用”（靠 Prompt 约束），要么是“没检索到但模型硬编”（靠检索质量+拒绝回答机制）。"
},
{
